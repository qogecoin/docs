...
.. warning::
   ...
   Running a web browser on the same node with a REST enabled ``qogecoind`` can
...
``8332`` for mainnet and port ``18332`` for testnet.
...
Given a transaction hash: returns a transaction in binary, hex-encoded binary or
JSON formats.
...
Given a block hash: returns ``<COUNT>`` amount of blockheaders in upward
direction.
...
for input and output serialisation, for example:
