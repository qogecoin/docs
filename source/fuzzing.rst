...
A special test harness ``test_qogecoin_fuzzy`` is provided to provide an easy
...
We disable ``ccache`` because we don't want to pollute the ccache with
...
Extract these (or other starting inputs) into the ``inputs`` directory before
starting fuzzing.
